...
d3.json('data/africaRev.geojson').then(geojson => {
  ...
  mapboxgl.accessToken = 'pk.eyJ1IjoiYW5kcmV3bGV2aW5zb24iLCJhIjoiY2pub3RxNXB2MDA5cTNxb2M5MjNoaHl5diJ9.Zq4eS5UJd_60fgNBAFiUsw';
  ...
  let colorScaleGreen = d3.scaleSequential(d3.interpolateGreens).domain([domainMin, areaMax]);

  // need to fix below color
  let colorScalePurple = d3.scaleSequential(d3.interpolateRdBu).domain([0, 50, 100]);
  ...
  let featureElement = svgAfrica
    ...
    .attr('fill', function (d) {
      return d.properties.totaltwh == 0 || d.properties.region_wb != 'Sub-Saharan Africa' ? '#696969' : colorScaleGreen(d.properties.totaltwh / d.properties.areakm); // per area
    })
  ...
  featureElement
    .on('mouseover', function (d) {
      tooltip.show(
        ...
        Access to Electricity: ${d.properties.access == 0 || d.properties.access == null ? '--' : d.properties.access + '%'}<br>
        Doing Business Rank: ${d.properties.easedb == 0 || d.properties.easedb == null ? '--' : numFormatT(d.properties.easedb) + ' of 190'}<br>
       `
      );
    })
    ...
    .on('active', function (i) {
      ...
      if (i == 1) {
        ...
        featureElement.attr('stroke', function (d) {
          if (d.properties.name == 'Gabon' || d.properties.name == 'Cape Verde' || d.properties.name == 'South Africa' || d.properties.name == 'Seychelles' || d.properties.name == 'Mauritius') {
            return '#00FF40';
          }
        });
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>${numFormatF(((d.properties.totaltwh / d.properties.pop_2017) * 1000000000) / 1000000)}</td>
            <td>${numFormatF((0.001380319312251 * 1000000000) / 1000000)}</td>
          </tr>
          <tr>
            <td class="left tick bottom">Per Km<sup>2</sup> (KWh)</td>
            <td class="bottom">${numFormatF(((d.properties.totaltwh / d.properties.areakm) * 1000000000) / 1000000)}</td>
            <td class="bottom">${numFormatF((0.045678932691726 * 1000000000) / 1000000)}</td>
        </tbody>`;
          ...
          analysisP.innerHTML = `Let's put this in perspective...<br><br>In a single year, there's <span class="green large-pos"><strong>${numFormatT(
            ((d.properties.totaltwh / d.properties.areakm) * 1000000000) / (12984 * 78.69)
