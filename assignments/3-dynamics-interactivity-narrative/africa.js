...
d3.json("africaTopoMap.json", function(error, data) {
  ...
  var map = svgAfrica.append("g").attr("class", "map");
  ...
  var countries = map
    ...
    .style("fill", function(d, i) {
      return colorScale(d.properties.pop_est);
    });
  ...
  countries.on("mouseover", function(d, i) {
    ...
    tooltip.show(
      "<b>" +
        d.properties.subunit +
        "</b>" +
        "<br>" +
        "Population: " +
        numFormat(d.properties.pop_est)
    );
    //toGreyExcept(this);
  });
  ...
  map
    .append("path")
    .datum(
      topojson.mesh(data, data.objects.collection, function(a, b) {
        return a !== b;
      })
    )
    .attr("d", path)
